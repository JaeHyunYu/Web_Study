...
function Movie({ medium_cover_image, title, summary, genres, id }) {
    return (
        <div className={styles.container}>
            ...
            <img alt="no IMG" className={styles.img} src={medium_cover_image} alt={title} />
            <h2 className={styles.header}>
                <Link style={{ textDecoration: 'none' }} to={`/movie/${id}`}>{title}</Link>
        ...
        summary는 string이기때문에 배열method 사용가능(slice method는 0~235까지 배열을 의미*/}
            <div className={styles.show}>
                {
                    (genres == undefined ? <li className={styles.list}> No Genres </li> :
                        <ul>
                            {genres.map((g) => (<li className={styles.list} key={g}>{g}</li>)
                            )}
